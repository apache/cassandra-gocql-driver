...
func (c *controlConn) shuffleDial(endpoints []*HostInfo) (*Conn, error) {
	...
	for _, host := range shuffled {
		...
		c.session.cfg.Logger.Printf("gocql: unable to dial control conn %v: %v\n", host.ConnectAddress(), err)
	}
...
func (c *controlConn) reconnect(refreshring bool) {
	...
	if err := c.setupConn(newConn); err != nil {
		newConn.Close()
		c.session.cfg.Logger.Printf("gocql: control unable to register events: %v\n", err)
		return
	}
...
func (c *controlConn) query(statement string, values ...interface{}) (iter *Iter) {
	...
	for {
		...
		if gocqlDebug && iter.err != nil {
			c.session.cfg.Logger.Printf("control: error executing %q: %v\n", statement, iter.err)
		}
