...
func TestControlConn_ReconnectRefreshesRing(t *testing.T) {
	...
	assertNodeUp := func() error {
		...
		if len(hosts) != len(cassNodes) {
			return fmt.Errorf("expected %v hosts in ring but there were %v", len(cassNodes), len(hosts))
		}
		for _, host := range hosts {
			if !host.IsUp() {
		...
		if poolsLen != len(cassNodes) {
			return fmt.Errorf("expected %v connection pool but there were %v", len(cassNodes), poolsLen)
		}
		return nil
	}
