...
import (
	"errors"
	"fmt"
	"github.com/gocql/gocql/compressor"
...
func (f *framer) readFrame(r io.Reader, head *FrameHeader) error {
	...
	if head.flags&flagCompress == flagCompress {
		if f.compres == nil {
			return session.NewErrProtocol("no compressor available with compressed frame body")
		}
...
func (f *framer) parseFrame() (frame frame, err error) {
	...
	if f.header.version.request() {
		return nil, session.NewErrProtocol("got a request frame from server: %v", f.header.version)
	}
	...
	default:
		return nil, session.NewErrProtocol("unknown op in frame header: %s", f.header.op)
	}
...
func (f *framer) readTypeInfo() protocol.TypeInfo {
	...
	simple := protocol.NativeType{
		Proto: f.proto,
		Typ:   protocol.Type(id),
	}

	if simple.Typ == protocol.TypeCustom {
		simple.Cust = f.readString()
		if cassType := getApacheCassandraType(simple.Cust); cassType != protocol.TypeCustom {
			simple.Typ = cassType
		}
	}

	switch simple.Typ {
	case protocol.TypeTuple:
		n := f.readShort()
		tuple := protocol.TupleTypeInfo{
			NativeType: simple,
			Elems:      make([]protocol.TypeInfo, n),
		}
	...
	case protocol.TypeUDT:
		udt := protocol.UDTTypeInfo{
			NativeType: simple,
		}
		...
		n := f.readShort()
		udt.Elements = make([]protocol.UDTField, n)
		for i := 0; i < int(n); i++ {
		...
		return udt
	case protocol.TypeMap, protocol.TypeList, protocol.TypeSet:
		collection := protocol.CollectionType{
			NativeType: simple,
		}

		if simple.Typ == protocol.TypeMap {
...
func (f *framer) parsePreparedMetadata() preparedMetadata {
	...
	var cols []session.ColumnInfo
	if meta.colCount < 1000 {
		// preallocate columninfo to avoid excess copying
		cols = make([]session.ColumnInfo, meta.colCount)
...
func (f *framer) readCol(col *session.ColumnInfo, meta *resultMetadata, globalSpec bool, keyspace, table string) {
	if !globalSpec {
...
func (f *framer) parseResultFrame() (frame, error) {
	...
	return nil, session.NewErrProtocol("unknown result kind: %x", kind)
}
...
func (f *framer) writeQueryParams(opts *queryParams) {
	...
	if opts.serialConsistency > 0 {
		f.writeConsistency(consistency.Consistency(opts.serialConsistency))
	}
...
func (f *framer) writeBatchFrame(streamID int, w *writeBatchFrame, customPayload map[string][]byte) error {
	...
	if f.proto > protoVersion2 {
		...
		if w.serialConsistency > 0 {
			f.writeConsistency(consistency.Consistency(w.serialConsistency))
		}
...
func (f *framer) readUUID() *protocol.UUID {
	if len(f.buf) < 16 {
	...
	u, _ := protocol.UUIDFromBytes(f.buf[:16])
	f.buf = f.buf[16:]
	return &u
}
...
func (f *framer) readConsistency() consistency.Consistency {
	return consistency.Consistency(f.readShort())
}
...
func (f *framer) writeConsistency(cons consistency.Consistency) {
