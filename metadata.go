...
func (t *typeParser) parse() (typeParserResult, bool, error) {
	...
	if strings.HasPrefix(ast.name, COMPOSITE_TYPE) {
		...
		if count == 0 {
			// a bare CompositeType with no parameter list is malformed; treat
			// it as an unparseable type rather than indexing params[-1]
			return typeParserResult{}, false, nil
		}
...
func (t *typeParser) parseParamNodes() (params []typeParserParamNode, ok bool) {
	...
	if t.index >= len(t.input) {
		// ran off the end without a closing ')': the parameter list is
		// unterminated, so report a parse failure instead of panicking
		return nil, false
	}
